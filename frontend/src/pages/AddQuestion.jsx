...
const AddQuestion = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await questionBankService.addQuestion(bankId, submitData);
      toast.success('Thêm câu hỏi thành công!');
      navigate(`/question-banks/${bankId}`);
    } catch (error) {
      console.error('Add question error:', error);
      toast.error(error.response?.data?.detail || 'Không thể thêm câu hỏi');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <h1 className="text-3xl font-bold text-gray-900 mb-8">Thêm câu hỏi mới</h1>
